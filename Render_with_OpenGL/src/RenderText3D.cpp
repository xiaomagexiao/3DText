...
enum Uniform_IDs { lights,
                   VPmatrix,
                   NumUniforms };
enum Attrib_Ids { vPostion,
                  vNormal,
                  vTexCoord };
...
int main() {
    ...
    for (const auto &c : text) {
        ...
        std::string fontPath(ROOT_PATH);
        fontPath.append("/fonts/simhei.ttf");
        FreeTypeFont temp_char(c, (GLchar *)fontPath.c_str());
        //����ÿ�����εĲ�������
    ...
    std::string vs(ROOT_PATH);
    vs.append("/Render_with_OpenGL/shader/text3D.vert");
    
    std::string fs(ROOT_PATH);
    fs.append("/Render_with_OpenGL/shader/text3D.frag");
    Shader textShader(vs, fs);
...
//        glBindBuffer(GL_UNIFORM_BUFFER, 0);
        ...
        textShader.setUniformMat4("projection", proj);
